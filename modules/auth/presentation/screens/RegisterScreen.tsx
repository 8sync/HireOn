      ...
      <div className={`glass-panel fade-in ${styles.authCard}`}>
        ...
        <form style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <Link href={AppRoutes.dashboard} style={{ width: '100%' }}>
            <button type="button" className="btn-primary" style={{ width: '100%', marginTop: '8px', padding: '14px' }}>Sign Up</button>
          </Link>
        </form>
        ...
        <div className={styles.socialButtons}>
          ...
          <button className={styles.socialBtn} aria-label="Sign Up with Meta">
            <MetaIcon />
          </button>
        </div>

        <div className={styles.footerLink}>
          Already have an account? <Link href={AppRoutes.auth.login} className={styles.link}>Sign in</Link>
        </div>
      </div>
    </div>
  );
